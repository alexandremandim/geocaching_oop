   ...
   private static boolean login(UsersGroup utilizadores){
    ...
    private static boolean registerUser(UsersGroup utilizadores){
        ...
        while(erro ==1){
            ...
            nome = input.next();
            u = utilizadores.existeUser(nome);
            
            if(utilizadores.get(email) != null || u!= null){
                if(u!= null){
    ...
    private static void encontreiCache(TreeSet <Cache> treeCaches){
        ...
        for(Cache c: treeCaches){   /* verifica se existe essa cache */
    ...
    private static void adicionarUtilizadorAmigos(UsersGroup users){
        User u;
        ...
        u = users.existeUser(user);
        if(u == null){
            System.out.println("Esse utilizador nao existe:");
            return;
        };
        if(logged.getAmigos().contains(u) == true){
            System.out.println("O utilizador " + user + " ja é seu amigo");
            return;
        }
        else{
            logged.adicionarAmigo(u);
            System.out.println("O utilizador " + user+ " é agora seu amigo");
        }
    }
    ...
    private static void testes(UsersGroup users, TreeSet <Cache> treeCaches){
        ...
        newUser = new User("kiko", "pass", "Francisco", 'F', "ruadoze", 0, new GregorianCalendar(1995,1,31));
        users.insertUser(newUser);
        
        newUser = new User("rafa", "pass", "Rafael", 'F', "ruadogirassol", 0, new GregorianCalendar(1995,1,23));
        users.insertUser(newUser);
        
        newUser = new User("alex", "a", "Alex", 'M', "ruadasfloress", 0, new GregorianCalendar(1995,12,20));
        users.insertUser(newUser);
               ...
               "10. Ver estatística mensalmente ou anualmente;" +"\n" + "11. Adicionar um utilizador aos amigos\n" + "12. Remover um utilizador dos amigos\n" + "13. Ver todos os users\n" + "14. Ver todas as caches\n" + "0. Sair;");
                ...
                switch(choice){
                    ...
                    case 3: /* Registar descoberta de cache */
                        ...
                        break;
                    case 4:
                        removerEncontroDeCache(treeCaches);
                        break;
                    case 5: /* Ver nossas ultimas 10 atividades */
                        ArrayList <Registo> ultimasAtividades = logged.getRegistos();
                        Registo aux;
                        Iterator <Registo> iterador = ultimasAtividades.iterator();
                        int z = 0;
                        
                        while(iterador.hasNext() && z < 10){
                            aux =  iterador.next();
                    ...
                    case 11:/* Adicionar um utilizador aos amigos*/
                        ...
                        break;
                    case 12:
                        ArrayList<User> todos = users.todosUsers();
